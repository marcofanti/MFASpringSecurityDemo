...
public class OTPController {
	...
	/*				} else {
				...
				SecurityContextHolder.clearContext();
				HttpSession hs = request.getSession();
				Enumeration enum2 = hs.getAttributeNames();
	...
	private static ModelAndView addReport(ModelAndView modelAndView, OTPModel otpModel, Report bhsReport, String type, int iteration, String viewName) {
		...
		modelAndView.addObject("pocAnomaly", "<b>Cut and Paste Detected: </b>" + bhsReport.isPocAnomaly());
	...
	private boolean evaluateReport(Report bhsReport, String phase, int minimumStepUpScoreValue, int maximumStepUpRiskValue) {
		log.error("bhsReport.getScore() : " + bhsReport.getScore() + " minimumStepUpScoreValue: " + minimumStepUpScoreValue);
		log.error("bhsReport.getRisk() : " + bhsReport.getRisk() + " maximumStepUpRiskValue: " + maximumStepUpRiskValue);
		log.error("bhsReport.isPocAnomaly() : " + bhsReport.isPocAnomaly());
		if (bhsReport != null && (int) bhsReport.getScore() > minimumStepUpScoreValue && (int) bhsReport.getRisk() < maximumStepUpRiskValue && !bhsReport.isPocAnomaly()) {
			return true;
		} else {
			return false;
		}
